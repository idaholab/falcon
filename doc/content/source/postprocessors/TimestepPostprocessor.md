...
`targetpostprocessor` reaches `enthalpy_relative_tolerance`, an internal trigger latches; the
latch is permanent for the rest of the simulation and is preserved across restarts. Before the
trigger latches, the reported value is `dt_before_trigger` (default
