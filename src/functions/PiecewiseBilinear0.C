...
#include <stdlib.h>
using namespace std;
registerMooseObject("FalconApp", PiecewiseBilinear0);

defineLegacyParams(PiecewiseBilinear0);

InputParameters
PiecewiseBilinear0::validParams()
{
...
PiecewiseBilinear0::PiecewiseBilinear0(const InputParameters & parameters)
  : Function(parameters),
    ...
    _radial(getParam<bool>("radial"))
{
  ...
  if (_radial && (!_yaxisValid || !_xaxisValid))
    mooseError(
        "In PiecewiseBilinear0 ", _name, ": yaxis and xaxis must be defined when radial = true");
  ...
  if (!_data_file_name.empty())
  {
    if (parameters.isParamValid("x") || parameters.isParamValid("y") ||
      ...
      mooseError("In PiecewiseBilinear0: Cannot specify 'data_file' and 'x', 'y', or 'z' together.");
    else
      parse(x, y, z);
  }
             ...
             parameters.isParamValid("z")))
    mooseError(
        "In PiecewiseBilinear0: 'x' and 'y' and 'z' must be specified if any one is specified.");
  ...
  else
  {
    ...
    if (z_vec.size() != x.size() * y.size())
      mooseError("In PiecewiseBilinear0: Size of z should be the size of x times the size of y.");
...
PiecewiseBilinear0::~PiecewiseBilinear0() {}

Real
PiecewiseBilinear0::value(Real t, const Point & p) const
{
  Real retVal(0);
...
void
PiecewiseBilinear0::parse(std::vector<Real> & x, std::vector<Real> & y, ColumnMajorMatrix & z)
{
  std::ifstream file(_data_file_name.c_str());
  if (!file.good())
    mooseError("In PiecewiseBilinear0 ", _name, ": Error opening file '" + _data_file_name + "'.");
  std::string line;
  std::string stri;
  ...
  if (data.size() != offset)
    mooseError("ERROR! Inconsistency in data read from '" + _data_file_name +
               "' for PiecewiseBilinear0 function.");
}
