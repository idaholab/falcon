...
template<>
InputParameters validParams<FracturesFluidFlow>()
{
...
void FracturesFluidFlow::computeProperties()
{
  ...
  {  
    ...
    if (_has_enthalpy)
    {
      ...
      FracturesFluidFlow:: compute2PhProperties0( _permeability[qp], sat_fraction_out, dens_water_out, dens_steam_out, visc_water_out, visc_steam_out, _tau_water[qp], _tau_steam[qp]);
...
void FracturesFluidFlow::compute2PhProperties0(Real _per, Real  _Sw, Real _Denw, Real _Dens, Real _visw, Real _viss, Real& _watertau, Real  &_steamtau)
{
