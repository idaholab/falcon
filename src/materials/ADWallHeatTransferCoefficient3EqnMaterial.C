...
{
  using std::max, std::pow, std::cbrt;
  // Eq. 6-20
...
Gnielinski(ADReal Pr, ADReal Re)
{
  ...
  ADReal f2 = f * 0.5;
  // Eq. 6-9
  return (f2 * (Re - 1000) * Pr) / (1 + 12.7 * pow(f2, 0.5) * (pow(Pr, 2. / 3.) - 1));
}

void
ADWallHeatTransferCoefficient3EqnMaterial::computeQpProperties()
{
  using std::max, std::abs, std::pow;
  ...
  // Eq. 6-17
  ADReal Gr = max(THM::Grashof(beta, dT, _D_h[_qp], rho_film, _mu[_qp], _gravity_magnitude), 1.e-20);
