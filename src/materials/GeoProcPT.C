...
InputParameters validParams<GeoProcPT>()
{
  ...
  "compressibility",
  1.0e-5,
  "Total compressibility of the researvoir");
  ...
  "gx",
  0.0,
  "X component of the gravity pressure vector");

  params.addParam<Real>(
  "gy",
  0.0,
  "Y component of the gravity pressure vector");

  params.addParam<Real>(
  "gz",
  1.0,
  "Z component of the gravity pressure vector");
  ...
  "initial_density",
  1000,
  "Initial water density [kg/m^3]");

  params.addParam<Real>(
  "initial_viscosity",
  0.12e-3,
  "Initial water viscosity [Pa.s]");

  params.addParam<UserObjectName>(
  "water_steam_properties",
  "IAPWS-97 water-steam EOS, provide if temp_dependent_fluid_props = true");

  params.addParam<Real>(
  "specific_heat_rock",
  0.92e3,
  "Specific heat of the rock [J/(kg.K)]");
...
GeoProcPT::GeoProcPT(const std::string & name, InputParameters parameters):
   ...
   _has_chem(getParam<bool>("has_chem")),
   ...
   _igfox(getParam<Real>("gx")),
   _igfoy(getParam<Real>("gy")),
   _igfoz(getParam<Real>("gz")),

   _poro_old(_has_chem ? &declarePropertyOld<Real>("porosity") : NULL),
   ...
   _gvec(declareProperty<RealVectorValue>("gravity_vector")),
...
void GeoProcPT::computeProperties()
{
...
//this will need to be greatly expanded and turned into a function call
        if (_pressure_dependent_permeability == true)
        {
          ...
          _permeability[qp] = GeoProcPT::computePressurePermeability(_num,  _input_permeability, _pressure[qp], _initial_pressure, _c, _total_stress);
    ...
    if (_has_enthalpy)
    {
      ...
      GeoProcPT::compute2PhProperties0( _permeability[qp], _sat_fraction, _dens_water, _dens_steam, _visc_water, _visc_steam, _tau_water[qp],_tau_steam[qp]);
      ...
      GeoProcPT:: compute2PhProperties0( _permeability[qp], sat_fraction_out, dens_water_out, dens_steam_out, visc_water_out, visc_steam_out, _tau_water[qp], _tau_steam[qp]);
...
Real GeoProcPT::computePressurePermeability(int _num,  Real _input_permeability, Real _pressure, Real _initial_pressure, Real _c, Real _total_stress)
{
  Real _perm;
...
void GeoProcPT::compute2PhProperties0(Real _per, Real  _Sw, Real _Denw, Real _Dens, Real _visw, Real _viss, Real& _watertau, Real  &_steamtau)
{
