...
StochasticChemicalReactions::StochasticChemicalReactions(const std::string & name,
...
StochasticChemicalReactions::computeProperties()
{
  ...
  for(unsigned int qp=0; qp<_qrule->n_points(); qp++)
  {
      ...
      _permeability[qp] = _input_permeability[qp] * (1.0-_input_porosity) * (1.0-_input_porosity) * std::pow(_porosity[qp]/_input_porosity,3)/(1.0-_porosity[qp])/(1.0-_porosity[qp]);
