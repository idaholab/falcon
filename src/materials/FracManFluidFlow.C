...
template<>
InputParameters validParams<FracManFluidFlow>()
{
...
FracManFluidFlow::FracManFluidFlow(const std::string & name, InputParameters parameters) :
...
void FracManFluidFlow::computeProperties()
{
  ...
  {  
    ...
    if (_has_enthalpy)
    {
      ...
      FracManFluidFlow::compute2PhProperties0( _permeability[qp], _sat_fraction, _dens_water, _dens_steam, _visc_water, _visc_steam, _tau_water[qp],_tau_steam[qp]);
      ...
      FracManFluidFlow:: compute2PhProperties0( _permeability[qp], sat_fraction_out, dens_water_out, dens_steam_out, visc_water_out, visc_steam_out, _tau_water[qp], _tau_steam[qp]);
...
void FracManFluidFlow::compute2PhProperties0(Real _per, Real  _Sw, Real _Denw, Real _Dens, Real _visw, Real _viss, Real& _watertau, Real  &_steamtau)
{
