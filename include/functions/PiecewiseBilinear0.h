...
template <>
InputParameters validParams<PiecewiseBilinear0>();

//c
//c PiecewiseBilinear0 reads a file containing a (x, y, z) data table 
//c where (x,y) is a descretized or measured point and z is the measured value at (x,y) point.
//c  Then, function PiecewiseBilinear0 will evalaute any (x',y') points by making 
...
class PiecewiseBilinear0 : public Function
{
public:
  static InputParameters validParams();

  PiecewiseBilinear0(const InputParameters & parameters);

  // Necessary for using forward declaration of BilinearInterpolation in std::unique_ptr
  virtual ~PiecewiseBilinear0();
